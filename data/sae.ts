export type SaeProject = {
  title: string;
  level: string;
  context: string;
  objectives: string[];
  role: string;
  reflection: string;
  skills: string[];
  proof?: {
    label: string;
    href: string;
    type: "image" | "pdf";
  };
};

export const saeProjects: SaeProject[] = [
  {
    title: "Posture professionnelle",
    level: "BUT 1",
    context:
      "Réalisation d’une capsule vidéo de présentation personnelle et professionnelle.",
    objectives: [
      "Structurer une présentation claire",
      "Valoriser son parcours",
      "Travailler la communication orale",
    ],
    role:
      "Rédaction du script, organisation des idées, préparation de la prise de parole et gestion du temps.",
    reflection:
      "Cette SAE m’a permis de mieux comprendre l’importance de la communication professionnelle et de la capacité à se présenter clairement.",
    skills: [
      "Communication",
      "Gestion du temps",
      "Organisation",
      "Prise de recul",
    ],
  },
  {
    title: "Outils RH",
    level: "BUT 2",
    context:
      "Diagnostic RH réalisé à partir de données liées au turnover, à l’absentéisme et aux besoins en compétences.",
    objectives: [
      "Analyser les effectifs",
      "Identifier les dysfonctionnements RH",
      "Proposer des actions adaptées",
    ],
    role:
      "Analyse des données RH, réflexion sur les besoins en compétences et participation aux recommandations.",
    reflection:
      "Cette SAE m’a appris à relier les problématiques RH à la performance globale d’une organisation.",
    skills: [
      "Diagnostic RH",
      "Analyse des besoins en compétences",
      "Solutions RH",
      "Vision stratégique",
      "Analyse des effectifs",
    ],
    proof: {
      label: "Visualiser la preuve",
      href: "/docs/preuve-sae2.jpg",
      type: "image",
    },
  },
  {
    title: "Audit RH & QVCT",
    level: "BUT 3",
    context:
      "Audit RH réalisé pour un Carrefour Market confronté à des problématiques de turnover et d’absentéisme.",
    objectives: [
      "Réaliser un diagnostic RH",
      "Identifier les causes des difficultés",
      "Construire un plan d’action RH",
    ],
    role:
      "Recherche, analyse, rédaction, participation aux solutions RH et préparation de la présentation orale.",
    reflection:
      "Cette SAE a renforcé ma posture de consultante RH en articulant diagnostic, analyse stratégique et plan d’action.",
    skills: [
      "QVCT",
      "Pilotage RH",
      "Plan d’action",
      "Développement RH",
      "Stratégie RH",
      "Amélioration continue",
      "Gestion des compétences",
    ],
    proof: {
      label: "Visualiser le rapport",
      href: "/docs/preuve-sae3.pdf",
      type: "pdf",
    },
  },
];
